...
class make_nx:
    ...
    def __init__(self, carrier):
        ...
        SEACOST = 800/9000/48 #$/t/km
        RAILCOST = SEACOST*0.8
        SEALOAD = 800/48*0.05 #$/t
        RAILOAD = 800/48*0.1 #$/t
        ...
        self.impedance_factors = {        
                            ...
                            'lng-shipping':         {'gas':LNG_TRANSFER*tperEJ['gas'],'coal':0,'oil':0},
                            'oilfields-pipelines':  {'gas':0,'coal':0,'oil':0},
                            'pipelines-pipelines':  {'gas':GAS_PIPELINE*tperEJ['gas'],'coal':0,'oil':OIL_PIPELINE*tperEJ['oil']},
                            'ports-pipelines':      {'gas':0,'coal':0,'oil':0},
                            'ports-shipping':       {'gas':SEALOAD*tperEJ['gas'],'coal':SEALOAD*tperEJ['coal'],'oil':SEALOAD*tperEJ['oil']},
                            ...
                            'railways-railways':    {'gas':0,'coal':RAILCOST*tperEJ['coal'],'oil':0},
                            'refineries-pipelines': {'gas':0,'coal':0,'oil':0},
                            'shipping-shipping':    {'gas':SEACOST*tperEJ['gas'],'coal':SEACOST*tperEJ['coal'],'oil':SEACOST*tperEJ['oil']},
    ...
    def _load_dfs(self):
        ...
        keys = [kk['name'] for kk in self.recipes[self.carrier]]
    ...
    def _fill_graph(self):
        ...
        for step in self.recipes[self.carrier]:
            ...
            self.G.add_edges_from(
                [
                    ((r[0]+dup_strs[0]).strip(),(r[1]+dup_strs[1]).strip(),{'z':r[2]*self.impedance_factors[step['name']][self.carrier]}) 
    ...
    def _prep_flow(self):
        ...
        if self.carrier=='coal':
            ...
            # Fix Fix fix
            source_nodes = [n for n in self.G.nodes if ('OilField' in n) or ('wellpads-N' in n)]
        ...
        self.dfs['cities']['demand'] = self.dfs['cities']['total_'+self.carrier+'_consumption'] / 1e9 # EJ/yr
        ...
        attrs = {city:{'D':demand} for city, demand in scope_cities}
        ...
        if self.carrier=='coal':
            self.dfs['powerstns'].loc[self.dfs['powerstns']['fuel1']=='Coal','demand'] = self.dfs['powerstns'].loc[self.dfs['powerstns']['fuel1']=='Coal','capacity_mw']*8760*.6 * 3.6 / 1e9 # EJ/yr
        elif self.carrier=='gas':
            self.dfs['powerstns'].loc[self.dfs['powerstns']['fuel1']=='Gas','demand'] = self.dfs['powerstns'].loc[self.dfs['powerstns']['fuel1']=='Gas','capacity_mw']*8760*.6 * 3.6 / 1e9 # EJ/yr
        elif self.carrier=='oil':
            self.dfs['powerstns'].loc[self.dfs['powerstns']['fuel1']=='Oil','demand'] = self.dfs['powerstns'].loc[self.dfs['powerstns']['fuel1']=='Oil','capacity_mw']*8760*.6 * 3.6 / 1e9 # EJ/yr
        ...
        attrs = {pwrstn:{'D':demand} for pwrstn, demand in scope_powerstns}
