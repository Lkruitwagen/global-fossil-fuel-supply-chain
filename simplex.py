...
def network_simplex(G, demand='demand', capacity='capacity', weight='weight', pos=None, diff_flow =1/10):
    ...
    C.extend(repeat(faux_inf, n)) # add weights of faux edges
    U.extend(repeat(faux_inf, n)) # add capacities of faux edges
    ...
    pos = {ii:vv for ii, (kk,vv) in enumerate(pos.items())}
    vmin=min(pi)
    vmax =max(pi)

    def visualise_network(title):
        fig, ax = plt.subplots(1,1, figsize=(12,12))
        

        ### visualise nodes and potentials
        #print ('pos',pos)
        #print ('e',e)


        ax.scatter([vv[0] for kk,vv in pos.items()],[vv[1] for kk,vv in pos.items()],c=pi, cmap='PiYG', vmin=vmin, vmax=vmax, s=150, zorder=3)
        for kk,vv in pos.items():
            ax.text(vv[0], vv[1], str(kk), fontdict={'color':'w'}, horizontalalignment='center', verticalalignment='center')
            ax.text(vv[0]+0.05, vv[1]-0.025, f'D:{D[kk]}', fontdict={'fontsize':8}, horizontalalignment='center', verticalalignment='center')

        pos[-1] = [np.mean([vv[0] for kk,vv in pos.items()]),np.mean([vv[1] for kk,vv in pos.items()])]

        # draw N_-1
        ax.scatter(pos[-1][0], pos[-1][1],c='k', s=150, zorder=3)
        ax.text(pos[-1][0], pos[-1][1], str(-1), fontdict={'color':'w'}, horizontalalignment='center', verticalalignment='center')
            
        segs, weights, linestyles, colors = [],[],[],[]

        for ii_e, (s, t, f) in enumerate(zip(S,T,x)):
            ...
            if ii_e in edge:
                ...
                col='gray'
            if f==0:
                ax.arrow(pos[s][0], pos[s][1], pos[t][0]-pos[s][0], pos[t][1]-pos[s][1],length_includes_head=True,color=col, lw=1, ls=':', zorder=0)
            else:
                ax.arrow(pos[s][0], pos[s][1], pos[t][0]-pos[s][0], pos[t][1]-pos[s][1],length_includes_head=True,color=col, lw=np.log(f), ls='-', zorder=0)
            ax.text((pos[s][0]+pos[t][0])/2, (pos[s][1]+pos[t][1])/2, f'x:{f}', fontdict={'fontsize':8,'color':col})

        ax.set_title(title)
        plt.show()
        del pos[-1]

    #visualise_network('initial layout')
    ...
    def reduced_cost(i):
        ...
        """
        # c = C[i] - (pi[S[i]] - pi[T[i]])  #  7 - 312
        c = C[i] * x[i] - (pi[S[i]] - pi[T[i]])  #  *x[i]   # min
    ...
    def find_entering_edges():
        ...
        while m < M:
            ...
            if c >= 0:
                ...
                # Entering edge found.
                if x[i] == 0:
                    p = S[i]  # source of edge i
                    q = T[i]  # target of edge i
                else:
                    p = T[i]  # source of edge i
                    q = S[i]  # target of edge i
                yield i, p, q
                m = 0
    ...
    def find_leaving_edge(Wn, We):
        ...
        j, s = min(zip(reversed(We), reversed(Wn)),
                   key=lambda i_p: residual_capacity(*i_p))  # j: edge, s: node
    ...
    # Pivot loop -> write this in cypher???
    for i, p, q in find_entering_edges():  # i: edge, p: source of edge, q: target of edge
        print ('i,p,q', i,p,q)
        Wn, We = find_cycle(i, p, q)  # get the cycle
        j, s, t = find_leaving_edge(Wn, We)  # get the constraining edge on the cycle
        print ('j,s,t',j,s,t)
        augment_flow(Wn, We, residual_capacity(j, s)) # update the flows
        ...
        #visualise_network(f'entering edge: {p},{q}; leaving edge: {s},{t}')
        print ('x',x)
        print ('pi',pi)

    #visualise_network(f'Final vis')
