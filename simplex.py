import logging, os, sys, time, pickle
...
def network_simplex(G, demand='demand', capacity='capacity', weight='weight'):
    ...
    logger.info(f'Initialising... {time.time()-tic}')
    ...
    faux_inf = max(abs(d) for d in D) + 10
    C.extend(repeat(faux_inf, n))
    U.extend(repeat(faux_inf, n))
    logger.info(f'faux inf {faux_inf}')
    ...
    logger.info(f'Initialised {time.time()-tic}')
    ...
    for i, p, q in find_entering_edges():
        ...
        if global_counter %1000 ==0:
            #print (x[0:10], C[0:10])
            flow_cost = np.nansum([c * x for c, x in zip(C[:e], x[:e])])  # sum(c * x for c, x in zip(C[:e], x[:e]))
